  ...
  async execute(client, message, args) {
    ...
    if(serverQueue) {
      ...
      return serverQueue.textChannel.send( new Discord.MessageEmbed()
        .setColor("YELLOW")
        .setAuthor('Queued',message.author.avatarURL({format : "png",dynamic : true}))
        .setTitle(song.title)
        .setURL(song.url)
    ...
    if(!serverQueue) message.client.queue.set(message.guild.id, queueConstruct)
     ...
     if (!serverQueue) {
      try {
        ...
        await channel.leave();
        return message.channel.send({embed: {"description": `Kanala giriş yapamıyorum.: ${error}`, "color": "YELLOW"}}).catch(console.error);
      }
    }
